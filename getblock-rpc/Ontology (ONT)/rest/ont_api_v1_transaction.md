---
title: /api/v1/transaction - Ontology
description: Example code for the /api/v1/transaction rest method. Сomplete guide on how to use /api/v1/transaction rest in GetBlock.io Web3 documentation.
---

### Parameters


`preExec` - query

integer

Set 0 if contract needs to be pre-executed, else 1

`content-type` - header

string

set to \`application/json\`

`Action` - body

string

Specifies action, set to "sendrawtransaction"

`Version` - body

string

set to "1.0.0"

`Data` - body

string

Data hex code

### Request

``` java
curl --location --request POST 'https://ont.getblock.io/mainnet/rest/api/v1/grantong/Adj7W5Z2hTeKH7YwJsfMzLuwiD671mvJ6X?preExec=1' 
--header 'x-api-key: YOUR-API-KEY' 
--header 'Content-Type: application/json' 
--data-raw '{"Action": "sendrawtransaction", "Version": "1.0.0","Data": ""}'
```

###  Response

``` java
{
    "Action": "sendrawtransaction",
    "Desc": "SUCCESS",
    "Error": 0,
    "Result": "22471ab3f4b4307a99f00c9a717dbf8b26f5bf63bf47f9c560477da8181de777",
    "Version": "1.0.0"
}
```

